    ...
    while (!ctx.CancellationToken.IsCancellationRequested)
    {
        ...
        try
        {
            ...
            foreach (var vehicle in await fiatClient.Fetch())
            {
                ...
                var lastUpdate = new HaSensor(mqttClient, $"{haDevice.Name}_LastUpdate", haDevice, false) { Value = DateTime.Now.ToString("dd/MM HH:mm:ss") };
...
IEnumerable<HaEntity> CreateInteractiveEntities(CoconaAppContext ctx, FiatClient fiatClient, SimpleMqttClient mqttClient, Vehicle vehicle,
  HaDevice haDevice)
{
    var updateLocationButton = new HaButton(mqttClient, $"{haDevice.Name}_UpdateLocation", haDevice, async button =>
    {
    ...
    var deepRefreshButton = new HaButton(mqttClient, $"{haDevice.Name}_DeepRefresh", haDevice, async button =>
    {
    ...
    var lightsButton = new HaButton(mqttClient, $"{haDevice.Name}_Light", haDevice, async button =>
    {
    ...
    var chargeNowButton = new HaButton(mqttClient, $"{haDevice.Name}_ChargeNOW", haDevice, async button =>
    {
    ...
    var hvacButton = new HaButton(mqttClient, $"{haDevice.Name}_HVAC", haDevice, async button =>
    {
    ...
    var lockButton = new HaButton(mqttClient, $"{haDevice.Name}_DoorLock", haDevice, async button =>
    {
    ...
    var unLockButton = new HaButton(mqttClient, $"{haDevice.Name}_DoorUnLock", haDevice, async button =>
    {
    ...
    var fetchNowButton = new HaButton(mqttClient, $"{haDevice.Name}_FetchNow", haDevice, async button =>
    {
...
{
    var compactDetails = vehicle.Details.Compact(haDevice.Name);
    ...
    haEntities.Add(new HaSensor(mqttClient, $"{haDevice.Name}_Charge_Duration", haDevice, false)
    {
      //  DeviceClass = "duration",
        Value = textChargeDuration,
    });

    haEntities.Add(new HaSensor(mqttClient, $"{haDevice.Name}_Charge_Endtime", haDevice, false)
    {
      //  DeviceClass = "duration",
    ...
    var tracker = new HaDeviceTracker(mqttClient, $"{haDevice.Name}_Location", haDevice)
    {
    ...
    var trackerTimeStamp = new HaSensor(mqttClient, $"{haDevice.Name}_Location_TimeStamp", haDevice, false)
    {
